...
void Draw::drawX(int x1,int y1){
    int thicken=bound*0.05+1;
    for (int i = (bound*0.10); i <(bound*0.90); ++i) {
        for (int j = 0; j <thicken ; ++j) {
            matrix[size*(i+x1*bound)+(i+(y1*bound))+j].green = 255;
            matrix[size*(i+x1*bound)+(i+(y1*bound))+j].red = 153;
            matrix[size*(i+x1*bound)+(i+(y1*bound))+j].blue = 51;
            matrix[size*((bound)+x1*bound)-size*(i)+(i+(y1*bound))+j].green = 255;
            matrix[size*((bound)+x1*bound)-size*(i)+(i+(y1*bound))+j].red = 153;
            matrix[size*((bound)+x1*bound)-size*(i)+(i+(y1*bound))+j].blue = 51;
        }
    }
}
void Draw::drawO(int x1,int y1){
    int thicken=(bound*0.03)+1;
    ...
    for (int i = 0; i <bound ; ++i) {
        for (int j = 0; j <bound ; ++j) {
            ...
            for (int k = 0; k <thicken ; ++k) {
                if(pow((((i-xmid)*(i-xmid)+(j-ymid)*(j-ymid)-((bound*0.5)*(bound*0.5)))),2)<=3000){
                    matrix[size*(i+x1*bound)+(j+(y1*bound))+k].green = 255;
                    matrix[size*(i+x1*bound)+(j+(y1*bound))+k].red = 153;
                    matrix[size*(i+x1*bound)+(j+(y1*bound))+k].blue = 51;
                }
            }
        }
    }
